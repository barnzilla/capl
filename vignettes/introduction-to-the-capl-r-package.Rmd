...
Physical literacy moves beyond just fitness, motor skill or motivation in isolation. The CAPL is unique in that it can assess the multiple aspects of physical literacy: daily behaviour, motivation and confidence, knowledge and understanding, and physical competence.
...
The `capl` R package contains tools that enables users to compute and visualize CAPL-2 (Canadian Assessment of Physical Literacy, Second Edition) scores and interpretations from raw data, all within the R environment without having to use the <a href="https://www.capl-eclp.ca" target="_blank">CAPL-2 website</a>.
...
Before using the `capl` R package, users must first import raw data. The `import_capl_data()` function enables users to import data from an Excel workbook into the R environment.
...
One of the coding philosophies behind the `capl` R package is to return NA values rather than be "noisy" and throw errors when data is missing or invalid. In order to implement this "quiet" approach, every `capl` function involves validation of data.
...
capl_demo_data$intrinsic_motivation_score <- get_intrinsic_motivation_score(
  why_are_you_active1 = capl_demo_data$why_are_you_active1,
  why_are_you_active2 = capl_demo_data$why_are_you_active2,
  why_are_you_active3 = capl_demo_data$why_are_you_active3
)
```
...
```{r warning = FALSE, message = FALSE}
capl_demo_data$cardiorespiratory_fitness_means_score <- get_binary_score(capl_demo_data$cardiorespiratory_fitness_means, c(2, "How well the heart can pump blood and the lungs can provide oxygen"))
```

```{r warning = FALSE, message = FALSE}
capl_demo_data$cardiorespiratory_fitness_means_score
```
...
```{r warning = FALSE, message = FALSE}
capl_demo_data$muscular_strength_means_score <- get_binary_score(capl_demo_data$muscular_strength_means, c(1, "How well the muscles can push, pull or stretch"))
```

```{r warning = FALSE, message = FALSE}
capl_demo_data$muscular_strength_means_score
```
...
capl_demo_data$ku_score <- get_ku_score(
  ...
  cardiorespiratory_fitness_means_score = capl_demo_data$cardiorespiratory_fitness_means_score,
  muscular_strength_means_score = capl_demo_data$muscular_strength_means_score,
